...
// NO need for SummaryHeaderKeys string unions now
// tests/FundScreener/fund-screener-table-headers.spec.ts
import { test, expect, Page, BrowserContext } from '@playwright/test';
import { Routes } from '../../utils/urls';
import { TabBarComponents } from '../../components/FundScreener/TabBarComponent';
import { MutualFundSearchComponent } from '../../components/FundScreener/MutualFundSearchComponent';
import { FiltersComponent } from '../../components/FundScreener/FiltersComponent';
import { MutualFundTableComponent } from '../../components/FundScreener/MutualFundTableComponent';
import { FundScreenerTableHeadersComponent } from '../../components/FundScreener/FundScreenerTableHeaderComponets';
import { findArrayDifferences } from '../../Utils/arrayUtils';
import { investorClassStockFunds } from '../../Data/stock-funds';
...
// --------------------------------------------------------
// Header definitions - what to validate in each tab
// --------------------------------------------------------
const summaryHeadersToTest = [
    { name: 'Fund Name', getLocator: (h: FundScreenerTableHeadersComponent) => h.fundName },
    { name: 'Overall Morningstar Rating', getLocator: (h: FundScreenerTableHeadersComponent) => h.overallMorningstarRating },
    { name: 'Risk Potential', getLocator: (h: FundScreenerTableHeadersComponent) => h.riskPotential },
    { name: 'w/ Waiver', getLocator: (h: FundScreenerTableHeadersComponent) => h.sevenDayYieldWithWaiver },
    { name: 'w/out Waiver', getLocator: (h: FundScreenerTableHeadersComponent) => h.sevenDayYieldWithoutWaiver },
    { name: 'Daily YTD', getLocator: (h: FundScreenerTableHeadersComponent) => h.dailyYTD },
    { name: '1Y', getLocator: (h: FundScreenerTableHeadersComponent) => h.oneYearReturn },
    { name: '5Y', getLocator: (h: FundScreenerTableHeadersComponent) => h.fiveYearReturn },
    { name: '10Y', getLocator: (h: FundScreenerTableHeadersComponent) => h.tenYearReturn },
    { name: 'Since Inception', getLocator: (h: FundScreenerTableHeadersComponent) => h.sinceInceptionReturn },
    { name: 'Gross', getLocator: (h: FundScreenerTableHeadersComponent) => h.grossExpenseRatio },
    { name: 'Net', getLocator: (h: FundScreenerTableHeadersComponent) => h.netExpenseRatio },
];


const categoryAndVolatilityHeadersToTest = [
    { name: 'Morningstar Category', getLocator: (h: FundScreenerTableHeadersComponent) => h.morningstarCategory },
    { name: 'Morningstar Sub-Category', getLocator: (h: FundScreenerTableHeadersComponent) => h.morningstarSubCategory },
    { name: 'Alpha', getLocator: (h: FundScreenerTableHeadersComponent) => h.alpha },
    { name: 'Beta', getLocator: (h: FundScreenerTableHeadersComponent) => h.beta },
    { name: 'Annual Turnover Rate', getLocator: (h: FundScreenerTableHeadersComponent) => h.annualTurnoverRate },
];

const pricesHeadersToTest = [
    { name: 'Current NAV $', getLocator: (h: FundScreenerTableHeadersComponent) => h.currentNAV },
    { name: 'NAV Change $', getLocator: (h: FundScreenerTableHeadersComponent) => h.navChange },
    { name: 'Change %', getLocator: (h: FundScreenerTableHeadersComponent) => h.changePercent },
    { name: 'NAV 12 Month Low - High $', getLocator: (h: FundScreenerTableHeadersComponent) => h.navLowHigh },
];


const yieldsHeadersToTest = [
    { name: 'w/ Waiver', getLocator: (h: FundScreenerTableHeadersComponent) => h.yieldsSevenDayWithWaiver },
    { name: 'w/out Waiver', getLocator: (h: FundScreenerTableHeadersComponent) => h.yieldsSevenDayWithoutWaiver },
    { name: 'Daily YTD', getLocator: (h: FundScreenerTableHeadersComponent) => h.yieldsDailyYTD },
    { name: 'Monthly YTD', getLocator: (h: FundScreenerTableHeadersComponent) => h.yieldsMonthlyYTD },
    { name: '1Y', getLocator: (h: FundScreenerTableHeadersComponent) => h.yieldsOneYearReturn },
    { name: '5Y', getLocator: (h: FundScreenerTableHeadersComponent) => h.yieldsFiveYearReturn },
    { name: '10Y', getLocator: (h: FundScreenerTableHeadersComponent) => h.yieldsTenYearReturn },
    { name: 'Since Inception', getLocator: (h: FundScreenerTableHeadersComponent) => h.yieldsSinceInceptionReturn },
    { name: 'Gross', getLocator: (h: FundScreenerTableHeadersComponent) => h.yieldsGrossExpenseRatio },
    { name: 'Net', getLocator: (h: FundScreenerTableHeadersComponent) => h.yieldsNetExpenseRatio },
    { name: '30-Day Annualized Dividend Yield', getLocator: (h: FundScreenerTableHeadersComponent) => h.yieldsThirtyDaysAnnualized },
    { name: 'w/ Waiver', getLocator: (h: FundScreenerTableHeadersComponent) => h.yieldsThirtyDayWithWaiver },
    { name: 'w/out Waiver', getLocator: (h: FundScreenerTableHeadersComponent) => h.yieldsThirtyDayWithoutWaiver },
];
...
test.describe('Fund Screener Table Data', () => {
    ...
    test.describe('Summary Tab', () => {
        test.beforeAll(async () => {
            await tabBarComponent.clickSummaryTab();
            await headerComponent.waitForTableReady();
        });

        // Dynamic tests for all Summary headers
        summaryHeadersToTest.forEach(({ name, getLocator }) => {
            test(`Verify visibility and name of header "${name}" in Summary Tab`, async () => {
                const headerLocator = getLocator(headerComponent);
    ...
    test.describe('Category & Volatility Tab', () => {
        ...
        categoryAndVolatilityHeadersToTest.forEach(({ name, getLocator }) => {
            test(`Verify visibility and name of header "${name}" in Category & Volatility Tab`, async () => {
                const headerLocator = getLocator(headerComponent);
                await expect(headerLocator).toBeVisible();
    ...
    test.describe('Prices Tab', () => {
        ...
        pricesHeadersToTest.forEach(({ name, getLocator }) => {
            test(`Verify visibility and name of header "${name}" in Prices Tab`, async () => {
                const headerLocator = getLocator(headerComponent);
                await expect(headerLocator).toBeVisible();
    ...
    //     yieldsHeadersToTest.forEach(({ name, getLocator }) => {
    //         test(` 1 Verify visibility and name of header "${name}" in Yields Tab`, async () => {
    //             const headerLocator = getLocator(headerComponent);
    //             await expect(headerLocator).toBeVisible();
